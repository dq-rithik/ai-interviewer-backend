...
class GroqProvider(BaseAIProvider):
    ...
    def _call_api(self, system_prompt: str, user_prompt: str) -> str:
        ...
        try:
            ...
            # If response_format causes an error on certain models, retry without it
            if response.status_code == 400 and "response_format" in response.text:
                del payload["response_format"]
                retry_res = requests.post(self.endpoint, headers=headers, json=payload, timeout=30)
                retry_res.raise_for_status()
                return retry_res.json()["choices"][0]["message"]["content"]
            raise Exception(f"Groq API call failed: {str(e)} - {response.text}")
